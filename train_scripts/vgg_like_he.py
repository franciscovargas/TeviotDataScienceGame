...
model = Sequential()
model.add(Convolution2D(32, 3, 3, border_mode='valid', input_shape=(3, 64, 64),init="he_normal" ))
model.add(ELU())
...
print "loading train"
#train = np.load("../data/pkl/trainX.npz")
...
# model.fit(x_tr,
#           to_categorical(y_tr-1,4), nb_epoch=35)
...
logger.debug('Saving labels in file "../data/csv_lables/sub12345.csv"')

# submission_example = pd.read_csv("../data/csv_lables/sample_submission4.csv")
# submission_example["label"] = results
# submission_example.to_csv("../data/csv_lables/sub12345.csv",index=False)
# logger.debug( "Submitted at: " + ("../data/csv_lables/sub12345.csv") )
