...
from sheduler import TriangularLearningRate
from callbacks import BaseLogger, LearningRateScheduler
...
def create_rbms(input_shape=(3, 64, 64), wfiles=[], ffile=None):
    ...
    for encoder, decoder in zip(encoded, decoded):
        ...
        rbm.compile(optimizer=sgd, loss='mean_squared_error',
                    metrics=['accuracy'])
...
if __name__ == '__main__':
    ...
    if pretrain:
        # load dataset
        schedule = TriangularLearningRate(lr=0.001, step_size=500, max_lr=0.01)
        lrs = LearningRateScheduler(schedule, mode='batch', logger=blogger)
        ...
        for encoder, decoder in zip(encoders, decoders):
            ...
            decoder.fit_generator(generator, samples_per_epoch=len(x), nb_epoch=nb_epochs_ptrain, callbacks=[lrs])
    ...
    if finetune:
        ...
        full.fit_generator(generator, samples_per_epoch=len(x_tr), nb_epoch=nb_epochs_train,
                           validation_data=(x_te, to_categorical(y_te-1,4)) if validate else None,callbacks=[lrs])
