...
def create_rbms(input_shape=(3, 64, 64)):
    ...
    # encoder
    x = Convolution2D(f1, k1, k1, border_mode='same', activation='relu',weights=[weights, np.array([0]*10) ]
)(input_img)
    ...
    x = Flatten()(encoded[2])
    x = Dense(400, W_regularizer=l2(0.05), activation='relu')(x)
    x = Dropout(0.5)(x)
...
if __name__ == '__main__':
    ...
    #full.fit(X_train,
     #               to_categorical(y_train,4), nb_epoch=40,
      #              validation_data=(X_test, to_categorical(y_test,4)))

    #print full.evaluate(X_test, to_categorical(y_test,4), batch_size=100)

    #full.fit(x_tr, to_categorical(y_tr-1,4), nb_epoch=20, batch_size=100)
    full.fit(x_tr,
                    to_categorical(y_tr-1,4), nb_epoch=100,
                    validation_data=(x_te, to_categorical(y_te-1,4)))

    print full.evaluate(x_te, to_categorical(y_te-1,4), batch_size=100)

    logger.debug( 'Done training.' )

    #logger.debug( 'Submitting...' )
    #submit(full, sub=0001)
