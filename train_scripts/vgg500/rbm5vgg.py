...
def create_rbms(input_shape=(3, 64, 64), wfiles=[], ffile=None):
    ...
    # encoder 3.1
    x = Convolution2D( f3, k3, k3, border_mode='valid', activation='relu', name='encoder21' )( encoded[-1] )
    encoded.append( BatchNormalization( axis=1 )( x ) )

    # decoder 3.1
    x = ZeroPadding2D( ( z3, z3 ) )( encoded[-1] )
    x = Convolution2D( f3, k3, k3, border_mode='valid', activation='relu', name='decoder21a' )( x )
    x = BatchNormalization( axis=1 )( x )
    decoded.append( Convolution2D( f2, 1, 1, border_mode='same', activation='sigmoid', name='decoder21b' )( x ) )
