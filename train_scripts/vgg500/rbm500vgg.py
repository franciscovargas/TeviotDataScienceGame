...
def create_rbms(input_shape=(3, 64, 64), wfiles=[], ffile=None):
    ...
    # Fully connected
    inp_size = (k2 - z2) * (k2 - z2) * f2
    nb_units1 = 1024

    encoded.append( Flatten()( x ) )

    # encoder 4.1
    x = Dense( nb_units1, activation='relu' )( encoded[-1] )
    e = BatchNormalization( axis=1 )( x )
    ...
    x = Dense( nb_units2, activation='relu' )( encoded[-1] )
    e = BatchNormalization( axis=1 )( x )
    ...
    x = Dense( nb_units3, activation='relu' )( encoded[-1] )
    e = BatchNormalization( axis=1 )( x )

    # decoder 4.3
    decoded.append( Dense( nb_units2, activation='sigmoid' )( e ) )
    encoded.append( Dropout( 0.5 )( e ) )
