...
model.add(Convolution2D(nb_filter=20,border_mode='valid',
                        nb_row=5, nb_col=5))
model.add(Activation('relu'))
model.add(MaxPooling2D(pool_size=(2, 2)))
...
model.fit_generator(generator,
                    samples_per_epoch=len(X_train), nb_epoch=50,
...
#send_results(model.evaluate(X_test, to_categorical(y_test,4), batch_size=100))
